...
function load(f::File{DataFormat{:EXR}})
    Base.invokelatest(checked_import(idOpenEXR).load, f)
end

function save(f::File{DataFormat{:EXR}}, args...)
    Base.invokelatest(checked_import(idOpenEXR).save, f, args...)
end
